 ...
 * 头像上传原图裁剪容器
 */
public class AvatarCutView extends RelativeLayout {
    //裁剪原图
    ...
    public AvatarCutView(Context context) {
        this(context, null);
    }

    public AvatarCutView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AvatarCutView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void init(Context context, AttributeSet attrs) {
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.AvatarCutView);

        //获取剪切框距离左右的边距, 默认为50dp
        mHorizontalPadding = array.getDimensionPixelSize(R.styleable.AvatarCutView_mHorizontalPadding,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50, getResources().getDisplayMetrics()));
        //获取裁剪框边框宽度，默认1dp
        int clipBorderWidth = array.getDimensionPixelSize(R.styleable.AvatarCutView_clipBorderWidth,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, getResources().getDisplayMetrics()));
        //裁剪框类型(圆或者矩形)
        int clipType = array.getInt(R.styleable.AvatarCutView_clipType, 1);
...
//                    checkBorder();
    ...
    private void setDeltaXYtoMatrix(float dx,float dy){
        ...
        if(rect.width() >= width - 2 * mHorizontalPadding) {
            ...
            if (dx < 0) {
                float tempX =  (width-mHorizontalPadding)-rect.right;
                dx = dx>tempX?dx:tempX;
            } else {
                float tempX = mHorizontalPadding - rect.left;
                dx = dx<tempX?dx:tempX;
            }
        }
        if (rect.height() >= height - 2 * mVerticalPadding) {
            if (dy < 0) {
                float tempY = height-mVerticalPadding-rect.bottom;
                dy = dy>tempY?dy:tempY;
            } else {
                float tempY = mVerticalPadding - rect.top;
                dy = dy<tempY?dy:tempY;
            }
        }
        matrix.postTranslate(dx,dy);
